...
class LhotseAudioQuestionAnswerDataset(torch.utils.data.Dataset):
    ...
    def __getitem__duplex_(self, cuts) -> dict[str, torch.Tensor | list[str] | dict]:
        ...
        if self.use_voice_prompt:
            voice_prompt, voice_prompt_lens = self.extract_voice_prompt(answer_audios, answer_audio_lens, voice_prompt_metadata, self.codec_sample_rate, self.voice_prompt_max_duration, self.repeat_short_voice_prompt)
    ...
    def extract_voice_prompt(self, audio_tensor, audio_lens, voice_prompt_metadata, sample_rate=22050, max_duration=3.0, repeat_short_voice_prompt=False):
        """
        ...
        Args:
            audio_tensor (torch.Tensor): Input audio tensor of shape [batch, time]
            audio_lens (torch.Tensor): Tensor to match dtype and device for voice_prompt_lens
        ...
        for i, prompt in enumerate(voice_prompt_metadata):
            ...
            # Extract the segment
            segment = audio_tensor[i, start_sample:end_sample]
        ...
        # Use audio_lens to set dtype and device for voice_prompt_lens
        voice_prompt_lens = torch.tensor(voice_prompt_lens, dtype=audio_lens.dtype, device=audio_lens.device)
        ...
        # Ensure voice_prompt has the same dtype and device as audio_tensor
        voice_prompt = voice_prompt.to(dtype=audio_tensor.dtype, device=audio_tensor.device)
